...
const PRESETS = {
  x: {
    'outside-left': (
      targetRect: Object,
      placeableRect: Object,
      rootRect: Object,
      offsets: Object,
    ) => {
      return {
        left:
          targetRect.left - placeableRect.width - rootRect.left + offsets.left,
      };
    },
    'outside-right': (
      targetRect: Object,
...
const calculatePreset = (
  ...
  const resultOffset = { left: preset.offsetX || 0, top: preset.offsetY || 0 };

  return Object.assign(
    {},
    PRESETS.x[preset.xAxis](targetRect, placeableRect, rootRect, resultOffset),
    PRESETS.y[preset.yAxis](targetRect, placeableRect, rootRect, resultOffset),
  );
};

class PlacerWrapperInner extends React.Component {
  render() {
...
export default class PlacerWrapper extends React.Component {
    ...
    if (typeof window !== undefined && presets.length > 1) {
      ...
      presets.some(preset => {
        ...
        let rect = Object.assign(
          { width: placeableRect.width, height: placeableRect.height },
          position,
        );
  ...
  _getPlaceableRect() {
    return this._placeableDOMNode
      ? this._placeableDOMNode.getBoundingClientRect()
      : null;
  }
  ...
  render() {
    ...
    return (
      ...
      >
        <PlacerWrapperInner ref={this._onChildrenMount}>
          {React.cloneElement(
