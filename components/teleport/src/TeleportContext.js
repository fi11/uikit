    ...
    return {
      teleport: {
        ...
        remove: (componentID: string, callback: () => void) => {
          this._shownComponents = this._shownComponents.filter(
            id => id !== componentID,
          );
        ...
        getContextLevel: (): number => {
          ...
          if (!data) {
            throw new Error(
              `TeleportContext: Context with id "${this
                ._componentID}" is not mount`,
            );
          }
